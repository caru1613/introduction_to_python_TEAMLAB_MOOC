...
def is_validated_morse_code(user_input):
    ...
    for input in user_input:
        if input not in "-. ":
            ...
            if was_space == False:
                char_code = get_char_code_dict()
                #print("mose code: "+ input)
                char = char_code.get(morse_code)
                if char == None:
...
def encoding_sentence(english_sentence):
    ...
    for english_c in english_sentence:
        if need_blank == True:
            result += " "
        ...
        if english_c == " ":
            ...
            result += encoding_character(english_c.upper())
            need_blank = True
            is_space = False
...
def main():
    ...
    while True:
        ...
        if is_help_command(cmd) == True:
            print(get_help_message())
        elif is_validated_morse_code(cmd) == True:
            print(decoding_sentence(cmd))
        elif is_validated_english_sentence(cmd) == True:
